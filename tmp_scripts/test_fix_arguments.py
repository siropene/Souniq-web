#!/usr/bin/env python
"""
Script para probar el fix de argumentos posicionales
"""
import os
import sys
import tempfile
...
print("=== TEST FIX ARGUMENTOS POSICIONALES ===")

def test_api_with_fixed_arguments():
    try:
        ...
        try:
            ...
            try:
                # Probar con argumentos posicionales (CORRECTO)
                print("🚀 Probando con argumentos POSICIONALES...")
                result = client.predict(
                    temp_file_path,  # Argumento posicional
                    api_name="/predict"
                )
                print("✅ SUCCESS: Argumentos posicionales funcionan!")
                print(f"📥 Resultado: {type(result)}")
                
            except Exception as e:
                if "key-word arguments" in str(e):
